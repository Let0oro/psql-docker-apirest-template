...
# Table of contents
--- 
1. [Docker configuration](#start-your-docker-configuration)
2. [API postgres configuration](#start-your-api)
--- 
...
***Note: this folders don't exist, but you can create this manually (this is not neccesary for this project, but you probably see it in the dockerfile tutorial), this is thir structure:

3. Manage your Dockerfile to obtain the image you want (f.e.: postgres):

- Open your compose.yaml file in an IDE or text editor.

- Replace all the code at this file for this other: 
...
services: 
  db: 
    ...
    environment:
      ...
      POSTGRES_DB: db123
  app:
    image: <name of your project>-server
    ports: 
      - 13000:3000
```
...
server.get("/api/", async (req, res) => { //! GET
    try {
...
server.post("/api/", async (req, res) => { //! POST
...
server.post("/api/setup", async (req, res) => { //!POST to create the table, this must be our first call, like the creation of a table in mongoDB but through command lines
  try {
    await pool.query(
